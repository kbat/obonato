...
# list of other standard permissions are in parts/omelette/Products/Five/permissions.zcml
    ...
    def items(self):
        ...
        if self.context.type_of_items == 'featured':
            ...
            results = catalog.searchResults({'portal_type' : ['irwilot.content.illustration'], 'Subject' : ('featured'), 'sort_on' : 'start', 'sort_order' : 'descending', 'sort_limit' : nimages})[:nimages]
        else:
            results = catalog.searchResults({'portal_type' : ['irwilot.content.illustration'], 'sort_on' : 'start', 'sort_order' : 'descending', 'sort_limit' : nimages})[:nimages]
